...
class SpendingListCell: UICollectionViewCell {
    
    let CELLID = "cell"
    var items:[SpendingDto] = []
    var sumAmount: Double {
        if items.count == 0 { return 0 }
        
        var sum:Double = 0
        for i in items { sum += i.amt }
        return sum
    }
    ...
    lazy var table: UITableView = {
        ...
        v.register(SpendingItemCell.self, forCellReuseIdentifier: CELLID)
        v.dataSource = self
        v.delegate = self
...
extension SpendingListCell : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return items.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CELLID, for: indexPath) as! SpendingItemCell
...
class SpendingItemCell : UITableViewCell {
    ...
    var detail: SpendingDetailView = {
        let v = SpendingDetailView()
        v.translatesAutoresizingMaskIntoConstraints = false
        return v
    }()
}
