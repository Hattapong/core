...
class SavingListsCell: UICollectionViewCell {
    
    let CELLID = "cell"
    var items:[GoldSavingDto] = []
    var sumAmount: Double {
        if items.count == 0 { return 0 }
        
        var sum:Double = 0
        for i in items { sum += i.savingAmt }
        return sum
    }
    ...
    lazy var table: UITableView = {
        ...
        v.register(SavingItemCell.self, forCellReuseIdentifier: CELLID)
        v.dataSource = self
        v.delegate = self
...
extension SavingListsCell : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return items.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CELLID, for: indexPath) as! SavingItemCell
...
class SavingItemCell : UITableViewCell {
    ...
    var detail: GoldSavingDetailView = {
        let v = GoldSavingDetailView()
        v.translatesAutoresizingMaskIntoConstraints = false
        return v
    }()
}
