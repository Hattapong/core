...
class WithdrawListCell: UICollectionViewCell {
    
    let CELLID = "cell"
    var items:[WithdrawingDto] = []
    var sumAmount: Double {
        if items.count == 0 { return 0 }
        
        var sum:Double = 0
        for i in items { sum += i.withdrawAmt }
        return sum
    }
    ...
    lazy var table: UITableView = {
        ...
        v.register(WithdrawItemCell.self, forCellReuseIdentifier: CELLID)
        v.dataSource = self
        v.delegate = self
...
extension WithdrawListCell : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return items.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CELLID, for: indexPath) as! WithdrawItemCell
...
class WithdrawItemCell : UITableViewCell {
    ...
    var detail: WithdrawDetailView = {
        let v = WithdrawDetailView()
        v.translatesAutoresizingMaskIntoConstraints = false
        return v
    }()
}
