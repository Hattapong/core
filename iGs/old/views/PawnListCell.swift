...
class PawnListCell: UICollectionViewCell {
    ...
    var sumAmount: Double {
        ...
        var sum:Double = 0
        for i in items { sum += i.amountget ?? 0 }
        return sum
    }
        ...
        AF.request(router).validate().responseJSON { (response) in
            ...
            case .success:
                do {
                    ...
                    let list = try decoder.decode([PawnDto].self, from:
    ...
    lazy var table: UITableView = {
        ...
        v.register(PawnDTCell.self, forCellReuseIdentifier: CELLID)
        v.dataSource = self
        v.delegate = self
...
extension PawnListCell : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return items.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CELLID, for: indexPath) as! PawnDTCell
...
class PawnDTCell : UITableViewCell {
    ...
    var detail: PawnCellDetail = {
        let v = PawnCellDetail()
        v.translatesAutoresizingMaskIntoConstraints = false
        return v
    }()
}
