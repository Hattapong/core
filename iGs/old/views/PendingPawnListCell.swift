...
class PendingPawnListCell: UICollectionViewCell {
    
    let CELLID = "cell"
    var items:[PendingPawnDto] = []
    var sumAmount: Double {
        if items.count == 0 { return 0 }
        
        var sum:Double = 0
        for i in items { sum += i.amountpay ?? 0 }
        return sum
    }
    ...
    func fetchData() {
        let router = Router.pawnPendingInterests
        ...
        AF.request(router).validate().responseJSON { (response) in
            ...
            case .success:
                do {
                    ...
                    let list = try decoder.decode([PendingPawnDto].self, from:
    ...
    lazy var table: UITableView = {
        ...
        v.register(PendingPawnDTCell.self, forCellReuseIdentifier: CELLID)
        v.dataSource = self
        v.delegate = self
...
extension PendingPawnListCell : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return items.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CELLID, for: indexPath) as! PendingPawnDTCell
...
class PendingPawnDTCell : UITableViewCell {
    ...
    var detail: PendingPawnCellDetail = {
        let v = PendingPawnCellDetail()
        v.translatesAutoresizingMaskIntoConstraints = false
        return v
    }()
}
